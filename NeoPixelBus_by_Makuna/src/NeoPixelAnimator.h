...
#undef min
#include <functional>
typedef std::function<void(const AnimationParam& param, const void * data)> AnimUpdateCallback;
...
public:
    ...
    void StartAnimation(uint16_t indexAnimation, uint16_t duration, AnimUpdateCallback animUpdate, void * data = nullptr);
...
private:
    struct AnimationContext
    {
        ...
        void StartAnimation(uint16_t duration, AnimUpdateCallback animUpdate, void * data)
        {
            _duration = duration;
            _remaining = duration;
            _fnCallback = animUpdate;
            _fnCallbackData = data;
